var isTwitterButtonActive = false;
var quotes = [
...
["Beauty is in the eye of the beholder.", "Margaret Hungerf ord"],
...
["Success is never final. Failure is newer fatal. It's courage that counts.", "John Wooden"],
...
function generateQuote () {
	var quotearray = quotes[Math.floor(Math.random()*149)];
	quote = quotearray[0];
	quotee = quotearray[1];
	return 0;
}
...
function displayQuote() {
	...
	window.setTimeout( function(){
  	...
  	$('#quotee').addClass('animated fadein');
	window.setTimeout( function(){
   $('#quotee').removeClass('animated fadein');
   }, 500);  
	$('#quote').html(quote);
	$('#quotee').html(quotee);
	$('#twitter').attr('href', 'http://twitter.com/home/?status="' + quote + '" - ' + quotee);
...
function fixSize() {
	if ($(window).width() < 415) {
		$('#quoteBox').css({"width":"95%", "left":"2.5%", "top":"5%"});	
	} else {
		$('#quoteBox').css({"width":"60%", "left":"20%", "top":"20%"});
	}
}
$(document).ready(function() {fixSize();});
$(window).resize(function() {fixSize();});
